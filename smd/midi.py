...
class MidiPlayer:
    def __init__(self, dll: Path):
        self.lib_path = str(dll.resolve())


        
        self.soundfont = str(MidiFiles.SOUNDFONT.value.resolve()).encode()
        self.midi = str(MidiFiles.MIDI.value.resolve()).encode()
        self.work_queue: multiprocessing.Queue[str] = multiprocessing.Queue()
        self.child = multiprocessing.Process(target=self.worker, args=(self.work_queue,))
        self.child.start()

    def worker(self, queue: "multiprocessing.Queue[str]"):
        self.CIntArray16 = ctypes.c_int * 16
        try:
            self.player_lib = ctypes.CDLL(self.lib_path)
        except OSError as e:
    ...
    def _set_range(self, start: int, end: int, state: int):
